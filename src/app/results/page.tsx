...
export default async function ResultsPage() {
  ...
  return (
    <div className="space-y-8">
      ...
      {matches.length === 0 && (
        <p className="text-white/40">No completed matches yet.</p>
      )}
      {matches.map((match) => (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-white/5">
                  ...
                  {match.stage !== 'GROUP' && (
                    <th className="px-4 py-2 text-left text-white/40 font-normal">Advance</th>
                  )}
                  ...
                  return (
                    <tr key={user.id} className="border-b border-white/5 last:border-0">
                      ...
                      <td className="px-4 py-2">
                        {single ? <>{single.value} {pointsBadge(single.pointsAwarded)}</> : <span className="text-white/20">—</span>}
                      </td>
                      <td className="px-4 py-2">
                        {double_ ? <>{double_.value} {pointsBadge(double_.pointsAwarded)}</> : <span className="text-white/20">—</span>}
                      </td>
                      <td className="px-4 py-2">
                        {exact ? <>{exact.value} {pointsBadge(exact.pointsAwarded)}</> : <span className="text-white/20">—</span>}
                      </td>
                      {match.stage !== 'GROUP' && (
                        <td className="px-4 py-2">
                          {advance ? <>{advance.predictedTeam} {pointsBadge(advance.pointsAwarded)}</> : <span className="text-white/20">—</span>}
                        </td>
                      )}
